...
git init
git branch -M main
git remote add origin https://github.com/F34RL35S0N3/SeaPedia.git
```
...
- `GET /api/driver/jobs/available` — requireRole DRIVER. Return DeliveryJobs where status=AVAILABLE. Include order { deliveryMethod, totalAmount, deliveryFee, deliveryAddress { city, province }, store { name }, _count { items } }. Compute potentialEarning = order.deliveryFee * 0.7.

- `GET /api/driver/jobs/:id` — requireRole DRIVER. Return job with full order detail (buyer address, items list, store info, delivery method).

- `POST /api/driver/jobs/:id/take` — requireRole DRIVER. 
  Use prisma.$transaction with optimistic check:
  1. Re-fetch job inside transaction, verify status is still AVAILABLE (else 409 "Pekerjaan sudah diambil driver lain")
  2. Update job: driverId = userId, status = TAKEN, takenAt = now
  3. Update order.status = SEDANG_DIKIRIM
  4. Create OrderStatusHistory { status: SEDANG_DIKIRIM, note: "Diambil oleh pengemudi" }
  Return updated job.

- `POST /api/driver/jobs/:id/complete` — requireRole DRIVER.
  Verify job.driverId === userId (else 403). Verify job.status === TAKEN (else 400).
  Use prisma.$transaction:
  1. earnedAmount = order.deliveryFee * 0.7
  2. Update job: status = COMPLETED, completedAt = now, earnedAmount
  3. Update order.status = PESANAN_SELESAI
  4. Create OrderStatusHistory { status: PESANAN_SELESAI, note: "Pengiriman dikonfirmasi selesai oleh pengemudi" }
  5. Create WalletTransaction for seller: look up seller's wallet (create if not exists), add (order.totalAmount - order.deliveryFee) as INCOME
  Return completed job.

- `GET /api/driver/jobs/history` — requireRole DRIVER. Return completed jobs for this driver with order info and earnedAmount.

- `GET /api/driver/earnings` — requireRole DRIVER. Return { totalEarnings: sum of earnedAmount, completedJobsCount, jobHistory }.

### STEP 5.4 — UPDATE BUYER/SELLER ORDER DETAIL

Update `GET /api/buyer/orders/:id` and `GET /api/seller/orders/:id` to include `deliveryJob` (with driver info if taken).

### STEP 5.5 — DRIVER FRONTEND PAGES

**`frontend/app/driver/jobs/page.tsx`**
- List of available jobs, each card showing: store name, delivery method badge, destination city, estimated earning
- "Ambil Pekerjaan" button per job → POST /api/driver/jobs/:id/take → confirm dialog first → success toast → refresh

**`frontend/app/driver/jobs/[id]/page.tsx`**
- Full job detail: order items, delivery address, buyer district
- If job.driverId === currentUser.id AND status === TAKEN: show "Konfirmasi Selesai" button
- Status badge

**`frontend/app/driver/history/page.tsx`**
- Table of completed jobs: order info, date completed, earned amount

**`frontend/app/driver/earnings/page.tsx`**
- Stats: total earnings (Rp X.XXX), completed jobs count
- Monthly earnings bar chart (recharts)

**Update `frontend/app/buyer/orders/[id]/page.tsx`**:
- If order has deliveryJob with driverId: show "Driver telah diassign" with delivery progress
- Status timeline includes SEDANG_DIKIRIM and PESANAN_SELESAI steps

**Update `frontend/app/seller/orders/[id]/page.tsx`**:
- Show "Menunggu Pengemudi" or "Dalam Pengiriman" status
- Show driver info when assigned

---

### ⏸️ WAIT — VERIFY LEVEL 5

- [ ] Create full order flow: register buyer, top up, add to cart, checkout
- [ ] Seller processes order → status becomes "Menunggu Pengirim"
- [ ] Register driver1@seapedia.com / Driver123! with role DRIVER
- [ ] Driver views available jobs → sees order just processed
- [ ] Driver takes job → order status changes to "Sedang Dikirim"
- [ ] Second driver tries to take same job → 409 error
- [ ] Driver confirms complete → status = "Pesanan Selesai"
- [ ] Buyer order history shows "Pesanan Selesai"
- [ ] Seller's wallet (or income) increased by totalAmount - deliveryFee
- [ ] Driver job history shows completed job with earned amount (deliveryFee * 0.7)
- [ ] Try accessing /api/driver/jobs/available without DRIVER role → 403
...
- `GET /api/admin/dashboard` — requireRole ADMIN. Return aggregated stats using Promise.all for parallel queries:
  - userCount + breakdown by role (count UserRole grouped by role)
  - storeCount
  - productCount (active)
  - orderCounts grouped by status
  - voucherCount + active count
  - promoCount + active count
  - deliveryJobCounts by status
  - overdueOrderCount (calculate based on SLA + simulatedDate — see overdue service)

- `GET /api/admin/users` — requireRole ADMIN. Return paginated users with roles
- `GET /api/admin/stores` — requireRole ADMIN. Return all stores with seller info and product count
- `GET /api/admin/products` — requireRole ADMIN. Return all products with store info, paginated
- `GET /api/admin/orders` — requireRole ADMIN. Return all orders with buyer, seller, store info, status
- `GET /api/admin/delivery-jobs` — requireRole ADMIN. Return all jobs with driver info and order info
- `GET /api/admin/overdue-orders` — requireRole ADMIN. Return orders that are past SLA
...
// SLA rules (in days): how many days from the status change time until order is overdue
export const SLA_DAYS: Record<string, number> = {
  INSTANT: 1,   // 1 day from when SEDANG_DIKIRIM was set
  NEXT_DAY: 2,  // 2 days from when MENUNGGU_PENGIRIM was set
  REGULAR: 5,   // 5 days from when MENUNGGU_PENGIRIM was set
};
...
export async function processOverdueOrders(simulatedNow: Date): Promise<{ processed: number; details: any[] }> {
  // Fetch all non-final orders
  const orders = await prisma.order.findMany({
    where: {
      status: { notIn: [OrderStatus.PESANAN_SELESAI, OrderStatus.DIKEMBALIKAN] },
    },
    include: {
  ...
  for (const order of orders) {
    // Determine SLA reference timestamp
    let slaReferenceStatus: OrderStatus;
    if (order.deliveryMethod === 'INSTANT') {
      slaReferenceStatus = OrderStatus.SEDANG_DIKIRIM;
    } else {
      slaReferenceStatus = OrderStatus.MENUNGGU_PENGIRIM;
    }

    const referenceEntry = order.statusHistory.find(h => h.status === slaReferenceStatus);
    if (!referenceEntry) continue; // Not yet at the SLA reference point

    const slaDays = SLA_DAYS[order.deliveryMethod];
    const deadline = new Date(referenceEntry.createdAt);
    deadline.setDate(deadline.getDate() + slaDays);

    if (simulatedNow <= deadline) continue; // Not yet overdue

    // Process as overdue using a transaction
    try {
      await prisma.$transaction(async (tx) => {
        ...
        await tx.walletTransaction.create({
          data: {
            walletId: buyerWallet.id, type: 'REFUND',
            amount: order.totalAmount,
            description: `Refund otomatis pesanan #${order.id.slice(-8)} (melewati SLA)`,
            orderId: order.id,
          }
        });

        // Reverse seller income if any (create INCOME_REVERSAL transaction)
        ...
        if (sellerWallet) {
          ...
          await tx.walletTransaction.create({
            data: {
              walletId: sellerWallet.id, type: 'INCOME_REVERSAL',
              amount: income,
              description: `Pembatalan pendapatan pesanan #${order.id.slice(-8)} (melewati SLA)`,
              orderId: order.id,
            }
          });
        }
...
### STEP 6.4 — ADMIN ENDPOINTS FOR OVERDUE

Add to `backend/src/controllers/admin.controller.ts`:

- `GET /api/admin/current-date` — Return { currentDate: simulatedDate or real date, isSimulated: boolean }
- `POST /api/admin/simulate-next-day` — requireRole ADMIN. Get current simulated date (or real date), add 1 day, save to SystemConfig. Run processOverdueOrders(newDate). Return { newDate, result: { processed, details } }.
- `POST /api/admin/reset-simulation` — requireRole ADMIN. Delete SIMULATED_DATE config. Return { message: "Simulasi waktu direset" }.

### STEP 6.5 — SETUP node-cron

In `backend/src/index.ts`, add after app.listen:
```typescript
...
cron.schedule('0 0 * * *', async () => {
  ...
  console.log(`[CRON] Overdue check ran. Processed: ${result.processed} orders`);
});
```

### STEP 6.6 — ADMIN FRONTEND PAGES

**`frontend/app/admin/dashboard/page.tsx`**
- Stats cards grid: Total Users, Total Stores, Total Products, Total Orders, Active Vouchers, Available Jobs, Overdue Orders
- Each card has icon and colored border
- Recent orders table (last 10)
- Quick links to sub-sections

**`frontend/app/admin/users/page.tsx`** — Users table with username, email, roles badges, created date

**`frontend/app/admin/stores/page.tsx`** — Stores table with name, seller, product count, created date

**`frontend/app/admin/products/page.tsx`** — Products with store, price, stock, status

**`frontend/app/admin/orders/page.tsx`** — All orders with buyer, seller, status, total, date

**`frontend/app/admin/delivery-jobs/page.tsx`** — Jobs with order info, driver (if taken), status

**`frontend/app/admin/overdue/page.tsx`** — Overdue orders table

**`frontend/app/admin/vouchers/page.tsx`**
- Table: code, type, value, min order, used/max, expires, status chip (Active/Expired/Exhausted)
- "Buat Voucher" button → modal form with all fields
- Click row → detail modal

**`frontend/app/admin/promos/page.tsx`** — Same pattern for promos (no maxUsage field)

**`frontend/app/admin/time-simulation/page.tsx`**
- Current date card (simulated or real, with label)
- "Maju 1 Hari" button → POST /api/admin/simulate-next-day
- Show result toast: "X pesanan diproses sebagai overdue"
- "Reset Simulasi" button
- Link to overdue orders

---

### ⏸️ WAIT — VERIFY LEVEL 6

- [ ] Admin dashboard shows all stats with real numbers
- [ ] Admin can create voucher/promo from UI
- [ ] Create an order, let seller process it, have driver take it
- [ ] Go to Admin → Time Simulation → click "Maju 1 Hari" multiple times until past SLA
- [ ] Check overdue orders page — shows the order
- [ ] Check buyer wallet — refund received, transaction shows REFUND entry
- [ ] Check product stock — restored
- [ ] Check seller's wallet — INCOME_REVERSAL transaction present
- [ ] Order status is DIKEMBALIKAN
- [ ] Click "Maju 1 Hari" again on same order → NOT processed again (idempotent)
- [ ] Reset simulation → date resets to real date

### GIT — LEVEL 6

```bash
git add .
git commit -m "feat: level 6 - admin monitoring dashboard, voucher/promo UI, overdue auto-refund with time simulation"
git push origin main
```
...
Install: `npm install sanitize-html` (already in package.json)
...
// Apply to: review comments, store names, product names/descriptions, address fields
```

Go through ALL controllers and ensure:

1. **SQL Injection**: All Prisma queries use model methods (findMany, create, update, etc.) — never use `prisma.$queryRaw` with string interpolation. If any $queryRaw exists, replace with tagged template literal: `prisma.$queryRaw\`SELECT...\``

2. **XSS**: Apply `sanitize()` to all string user inputs before saving: review comments, store name, product name, product description, address fields, voucher descriptions

3. **Input validation with Zod on EVERY endpoint** — add missing schemas. Common validators to add:
   - price: z.number().positive()
   - stock: z.number().int().min(0)
   - quantity: z.number().int().min(1).max(100)
   - rating: z.number().int().min(1).max(5)
   - discountValue: z.number().positive().max(100) for PERCENTAGE, max 1_000_000 for FIXED
   - phone: z.string().regex(/^[0-9+\-\s]{8,15}$/)
   - postalCode: z.string().regex(/^[0-9]{5}$/)

4. **Ownership guards** — add these checks where missing:
   - Buyer can only access their own orders (verify order.buyerId === req.user.userId)
   - Seller can only access their own store's orders (verify order.sellerId === seller's userId)
   - Driver can only complete their own jobs (verify job.driverId === req.user.userId)
   - Buyer can only manage their own cart and addresses

5. **Token blacklist** — already implemented in auth.middleware.ts. Verify it works.

6. **helmet()** — already in index.ts. Confirm it's there.

7. **Rate limiting** — already on auth routes. Add general rate limiter:
   ```typescript
...
Add JSDoc swagger comments to at least these key endpoints (use @swagger annotation):
...
Update `backend/prisma/seed.ts` to create all demo accounts:

```typescript
// Users:
// admin@seapedia.com / Admin123! → role: ADMIN
// seller1@seapedia.com / Seller123! → role: SELLER → Store: "Toko Elektronik Jaya" → 5 products
// buyer1@seapedia.com / Buyer123! → role: BUYER → wallet: Rp 5.000.000 → 1 saved address
// driver1@seapedia.com / Driver123! → role: DRIVER
// multi@seapedia.com / Multi123! → roles: BUYER + SELLER + DRIVER (no store yet)

// Vouchers:
// SAVE10 → PERCENTAGE 10%, min 50000, max 100 uses, expires +1 year
// HEMAT20000 → FIXED 20000, min 100000, max 50 uses, expires +1 year

// Promos:
// PROMO5 → PERCENTAGE 5%, no min, no usage limit, expires +1 year
```
...
Multi-role e-commerce platform: Buyers, Sellers, Drivers, and Admins.
...
### Driver Earning Rule
driverEarning = order.deliveryFee × 0.70 (70% of delivery fee)
...
### Overdue SLA
| Method | SLA Reference | Days |
|--------|--------------|------|
| INSTANT | From SEDANG_DIKIRIM | 1 day |
| NEXT_DAY | From MENUNGGU_PENGIRIM | 2 days |
| REGULAR | From MENUNGGU_PENGIRIM | 5 days |

Time simulation: Admin → POST /api/admin/simulate-next-day
...
Go through ALL frontend pages and verify:
- NO `dangerouslySetInnerHTML` used anywhere on user content
- All review comments, product descriptions, store names rendered as `{variable}` (React auto-escapes)
- Add frontend Zod validation schemas for all forms (not just backend)
- Show clear, user-friendly error messages from API responses

---

### ⏸️ WAIT — VERIFY LEVEL 7

- [ ] http://localhost:4000/api/docs — Swagger UI loads with all endpoints documented
- [ ] npm run seed runs cleanly and creates all 5 demo accounts
- [ ] Security test: POST /api/reviews with body `{ "reviewerName": "test", "rating": 5, "comment": "<script>alert(1)</script>" }` → saved and displayed as plain text `<script>alert(1)</script>` NOT as executed script
- [ ] SQL test: POST /api/auth/login with email `"' OR 1=1 --"` → returns 401 "Invalid credentials" (not a DB error)
- [ ] Try accessing /api/seller/products with BUYER token → 403
- [ ] Try accessing /api/admin/dashboard with SELLER token → 403
- [ ] Logout → try using old token → 401 "Token has been invalidated"
- [ ] All 5 demo accounts login successfully
- [ ] Multi-role account shows role selection page after login
- [ ] Go through full end-to-end flow from README testing guide
...
## ═══════════════════════════════════════
## BONUS — DEPLOYMENT (15 pts)
## ═══════════════════════════════════════

### Backend on Railway (free tier)

1. Go to railway.app → New Project → Deploy from GitHub → select seapedia repo
2. Set Root Directory: `backend`
3. Add environment variables:
   - `DATABASE_URL` = `file:./dev.db` (for SQLite on Railway volume)
   - `JWT_SECRET` = (generate a random 32-char string)
   - `PORT` = `4000`
   - `NODE_ENV` = `production`
4. Add build command: `npm install && npx prisma migrate deploy && npm run seed && npm run build`
5. Add start command: `npm start`

**ALTERNATIVELY (more stable):** Use Neon (neon.tech) free PostgreSQL:
- Create Neon project → copy connection string
- Change `backend/prisma/schema.prisma` provider from `sqlite` to `postgresql`
- Set `DATABASE_URL` to Neon connection string
- Run `npx prisma migrate deploy`

### Frontend on Vercel

1. Go to vercel.com → Import from GitHub → select seapedia repo
...
```bash
git add .
git commit -m "chore: deployment config and production README update"
git push origin main
```
...
Your commit history should show these commits (oldest to newest):

```
chore: initialize monorepo with backend (Express+Prisma+SQLite) and frontend (Next.js)
feat: level 1 - public marketplace, auth with multi-role, application reviews, dashboard shells
feat: level 2 - seller store management, product CRUD, public catalog connected to real data
feat: level 3 - buyer wallet, delivery addresses, cart with single-store rule, checkout with PPN 12%
feat: level 4 - voucher/promo discounts with stacking, seller order processing, buyer/seller reports
feat: level 5 - driver job workflow (find, take, complete) with earnings, delivery tracking
feat: level 6 - admin monitoring dashboard, voucher/promo UI, overdue auto-refund with time simulation
feat: level 7 - XSS sanitization, Zod validation, ownership guards, swagger docs, complete seed and README
chore: deployment config and production README update
```
...
1. Never skip the VERIFY section before running git commands
2. If a step fails or has errors, fix it before moving to the next step
3. Keep backend running on port 4000 and frontend on port 3000 during verification
4. If any migration fails, run `npx prisma migrate reset --force` and re-run all migrations
5. The SQLite database file `dev.db` is in `backend/prisma/dev.db` — never commit it (it's in .gitignore)
6. If node_modules causes issues, delete and run `npm install` again
7. All amounts/prices should be stored as Float in SQLite (Prisma handles this correctly)
8. Dates in SQLite are stored as ISO strings — always use `new Date()` when creating timestamps
